...
require(["esri/Map", "esri/views/MapView", "esri/layers/FeatureLayer", "esri/symbols/SimpleMarkerSymbol"], function(Map, MapView, FeatureLayer, SimpleMarkerSymbol) {
    ...
    function getLayer(layerIndex) {
        return map.layers.items[layerIndex];
    }

    getLayer(POINT_ID).outFields = ["HOUSEHOLDS", "STATE_FIPS"];
    getLayer(POINT_ID).popupTemplate = {
        title: "{HOUSEHOLDS}",
        content: "{STATE_FIPS}"
    }
    getLayer(GROUP_ID).outFields = ["FEMALES", "AGE_22_29", "STATE_FIPS"];
    getLayer(GROUP_ID).popupTemplate = {
        title: "{STATE_FIPS}",
        content: "Number of females population is {FEMALES}, number of population between 29 to 29 is {AGE_22_29}"
    }
    getLayer(COUNTIES_ID).outFields = ["NAME", "STATE_NAME", "CNTY_FIPS"];
    getLayer(COUNTIES_ID).popupTemplate = {
        title: "{NAME}",
        content: "{STATE_NAME}, {CNTY_FIPS}"
    }
    getLayer(STATES_ID).outFields = ["STATE_NAME", "STATE_FIPS", "SUB_REGION", "STATE_ABBR"];
    getLayer(STATES_ID).popupTemplate = {
        title: "{STATE_NAME}, {STATE_FIPS}",
        content: "{SUB_REGION}, {STATE_ABBR}"
    }

    blockPointLayerToggle.addEventListener("change", function() {
        getLayer(POINT_ID).visible = blockPointLayerToggle.checked;
    });
    blockGroupToggle.addEventListener("change", function() {
        getLayer(GROUP_ID).visible = blockGroupToggle.checked;
    });
    countiesToggle.addEventListener("change", function() {
        getLayer(COUNTIES_ID).visible = countiesToggle.checked;
    });
    stateToggle.addEventListener("change", function() {
        getLayer(STATES_ID).visible = stateToggle.checked;
    });
    ...
    filterPoints.addEventListener("click", function() {
        ...
        getLayer(POINT_ID).definitionExpression = expression;
    });
    ...
    filterGroup.addEventListener("click", function() {
        ...
        getLayer(GROUP_ID).definitionExpression = expression;
    });
    ...
    filterCounties.addEventListener("click", function() {
        ...
        getLayer(COUNTIES_ID).definitionExpression = expression;
    });
    ...
    filterState.addEventListener("click", function() {
        ...
        getLayer(STATES_ID).definitionExpression = expression;
    });
    ...
    refreshPoints.addEventListener("click", function() {
        getLayer(POINT_ID).definitionExpression = "";
    });
    refreshGroup.addEventListener("click", function() {
        getLayer(GROUP_ID).definitionExpression = "";
    });
    refreshCounties.addEventListener("click", function() {
        getLayer(COUNTIES_ID).definitionExpression = "";
    });
    refreshStates.addEventListener("click", function() {
        getLayer(STATES_ID).definitionExpression = "";
    });
});